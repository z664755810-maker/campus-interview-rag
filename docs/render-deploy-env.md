...
| `DASHSCOPE_API_KEY` | `sk-ws-H.EXYDHMP.7PU2.MEYCIQD_DrIRKXC175Qj4OsTAMRkKT4vMbMfQ8kYdbgvqR-EjwIhAPtuTp47Z_sbGLE4o5V_fGqfyqBCai6aabc8GRX_oZta` | 阿里百炼 API Key |
...
- 重建完成后服务启动，`main.py` 的 `_auto_seed()` 会发现 chroma_db 为空 →
  **自动用 DashScope 灌入 50 道样例题**，无需手动点「一键载入示例题库」。
...
| 点「一键载入示例题库」 | 返回"已索引 50 段"（**不再 429**） | 仍 429 → `EMBEDDING_PROVIDER` 没设成 `dashscope` |
